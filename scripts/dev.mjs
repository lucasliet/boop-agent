...
function showBanner(url, stable, webhookSyncState) {
  ...
  const footerMessage =
    webhookSyncState === "synchronized"
      ? "The webhook above was registered with Telegram automatically."
      : webhookSyncState === "disabled"
        ? "Automatic Telegram webhook registration is disabled. Run npm run telegram:webhook -- <url> after changing the public URL."
        : "Telegram webhook registration failed. Check the webhook log above, then run npm run telegram:webhook.";
...
async function autoRegisterWebhook(publicUrl) {
  if (!autoWebhook) return "disabled";
  const webhookUrl = `${publicUrl}/telegram/webhook`;
  const prefix = `${C.ngrok}webhook${C.reset} │ `;
  const child = spawn(nodeCmd, ["scripts/telegram-webhook.mjs", webhookUrl], {
    cwd: root,
    env: { ...process.env },
  });
...
let telegramWebhookRegistrationUrl = "";
let telegramWebhookRegistration = Promise.resolve();
function registerTelegramWebhookOnce(publicUrl) {
  if (telegramWebhookRegistrationUrl === publicUrl) return telegramWebhookRegistration;
  telegramWebhookRegistrationUrl = publicUrl;
  telegramWebhookRegistration = telegramWebhookRegistration.then(() =>
    autoRegisterWebhook(publicUrl),
  );
  return telegramWebhookRegistration;
}

async function registerTelegramWhenTunnelAppears() {
  await sleep(2500);
  const start = Date.now();
  while (Date.now() - start < 60000) {
    const publicUrl = await readNgrokUrl();
    if (publicUrl) {
      await registerTelegramWebhookOnce(publicUrl);
      return;
    }
    await sleep(1000);
  }
...
if (useNgrok && ngrokInstalled && !ngrokDomain) {
  registerTelegramWhenTunnelAppears().catch(() => {});
}
  ...
  .then(async ([, , , ngrokUrl]) => {
    if (useNgrok && ngrokInstalled) {
      if (ngrokUrl) {
        ...
        const webhookSyncState = await registerTelegramWebhookOnce(
      ...
      const webhookSyncState = await registerTelegramWebhookOnce(publicUrl);
...
${C.banner}${line}
  ...
  ⚠ No public tunnel configured. Telegram won't work until you expose
